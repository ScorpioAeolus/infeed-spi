 ...
 *
 * @author typhoon
 * @date 2024-08-15 19:56 Thursday
 */
@Slf4j
...
public class SpiCglibProxy<T extends ISpiProvider> implements MethodInterceptor {
    ...
    SpiCglibProxy(Class<T> spiClass) {
    ...
    public Object intercept(Object target, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
        ...
        if (ISpiChainProvider.class.isAssignableFrom(spiClass)) {
            return _invokeChain(filterSpiImplMap, spiConfigMap, methodProxy, args);
        }
        if (ISpiCommitProvider.class.isAssignableFrom(spiClass)) {
            return _invokeCommit(filterSpiImplMap, spiConfigMap, methodProxy, args);
        }
    ...
    private Object _invokeChain(Map<String, ? extends ISpiProvider> spiImplMap, Map<String, SpiConfig> spiConfigMap, MethodProxy method, Object[] args) throws Throwable {
        ...
        for (Map.Entry<String, ? extends ISpiProvider> entry : spiImplMap.entrySet()) {
            ...
            if (args != null && spiImpl.condition(currentResult)) {
                ...
                try {
                    currentResult = method.invoke(spiImpl, new Object[]{currentResult});
    ...
    private Object _invokeCommit(Map<String, ? extends ISpiProvider> spiImplMap, Map<String, SpiConfig> spiConfigMap, MethodProxy method, Object[] args) throws Throwable {
    ...
    private Object _invokeService(Map<String, ? extends ISpiProvider> spiImplMap, Map<String, SpiConfig> spiConfigMap, MethodProxy method, Object[] args) throws Throwable {
        List compositeResult = Lists.newArrayList();
